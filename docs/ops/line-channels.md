...
7. **友だち追加状態の確認**: Messaging API の `push` は友だち追加済みのユーザーにしか届かない。
   「友だち追加済みか」をアプリ側でどう扱うか（例: 追加前は O-2 バッジのみで気づかせ、追加後は
   push も併用する）は ADR-007（`docs/decisions/ADR-007-raw-userid-consent.md`）の決定を前提に
   別途設計する。本書は友だち追加**導線**の記録に留め、`push` 実装そのものはこの手順書の対象外。
